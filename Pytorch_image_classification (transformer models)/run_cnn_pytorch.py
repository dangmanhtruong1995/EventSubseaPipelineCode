...
def run_pytorch_image_classification_model(
        model_name, dataset_name, feature_extract,
         use_pretrained, n_epoch, is_train, result_folder):
    np.random.seed(0)
    torch.manual_seed(0)
    print("We're using =>", DEVICE)
#    root_dir = "/home/ubuntu/TRUONG/datasets/%s/" % (dataset_name)
...
#    }
    ...
    dataset_train = datasets.ImageFolder(
        root=pjoin(root_dir, "train"),
        transform=image_transforms["train"])
    dataset_val = datasets.ImageFolder(
        root=pjoin(root_dir, "val"),
        transform=image_transforms["val"])
    dataset_test = datasets.ImageFolder(
        root = pjoin(root_dir, "test"),
    ...
    plot_from_dict(
        get_class_distribution(dataset_train, idx2class),
        plot_title="Entire Dataset (before train/val/test split)")
    plt.savefig(pjoin(result_folder, "class_dist.png"))
    plt.close()
    ...
    plot_from_dict(
        get_class_distribution_loaders(train_loader, dataset_train, idx2class),
        plot_title="Train Set", ax=axes[0])
    plot_from_dict(
        get_class_distribution_loaders(val_loader, dataset_train, idx2class),
        plot_title="Val Set", ax=axes[1])
    plt.savefig(pjoin(result_folder, "class_dist_2.png"))
    plt.close()
...
#    model_list = ["NoisyStudent"]
    model_list = ["VisionTransformer", "MaxViT", "SwinTransformer"]
#    model_list = ["inception_v3"]
#    model_list = ["inception_v3", "squeezenet", "densenet"]
#    dataset_name = "Saibok_Total_balanced"
#    dataset_name = "DatasetA_overlay_removed"
    dataset_name = "DatasetB_overlay_removed"
